...
本地 Provider 必须依次完成 Key 写入、生产同款严格探针和独立激活；仅探针通过的 `READY` 配置仍不能签发运行授权。激活后，每个 Agent attempt 获得最长 15 分钟、精确绑定 tenant/project/run/profile/provider/credential/models/budget 的 DLRT 短期令牌。CLI 只访问 loopback Gateway，不取得上游 Key；Gateway 再从进程内保管区租用精确 Key，并复用生产 DNS/CNAME 固定、HTTPS 校验、重定向复检、模型白名单、输出限制、串行 usage claim 和计价账本。Attempt 完成、失败或取消都会撤销令牌。sidecar 重启会清空 Key、Provider 激活和运行授权，因此管理员必须重新写入 Key 并测试、激活。
