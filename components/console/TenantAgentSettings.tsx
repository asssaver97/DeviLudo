...
export function TenantAgentSettings() {
  ...
  async function createProfile(event: FormEvent<HTMLFormElement>) {
    ...
    await mutate("/api/settings/agents/profiles", "POST", {
      ...
      budgetUsd: Number(form.get("budget") ?? 25),
      maxTurns: 100,
      timeoutSeconds: 7200,
    ...
    <div className="agent-settings-grid">
      ...
      <section className="settings-card settings-card-wide">
        ...
        <form className="settings-form provider-grid" onSubmit={createProfile}>
          <label>Agent<select disabled={busy || readOnly} name="agent"><option value="claude-code">Claude Code（默认）</option><option value="codex-cli">Codex CLI</option></select></label>
          <label>固定 Installation<select disabled={busy || readOnly} name="installationId" required>{installations.map((item) => <option key={item.id} value={item.id}>{item.agent} · {item.id}</option>)}</select></label>
          ...
          <label>HTTPS Base URL<input disabled={busy || readOnly} name="baseUrl" placeholder="https://gateway.example.com/v1" required type="url" /></label>
          <label>精确模型 ID<input disabled={busy || readOnly} name="primaryModel" placeholder="禁止 latest / default / sonnet" required /></label>
          <label>任务预算（USD）<input defaultValue="25" disabled={busy || readOnly} max="100" min="1" name="budget" required type="number" /></label>
          ...
          <button className="button button-primary provider-submit" disabled={busy || readOnly || !credentials.length || !installations.length} type="submit">保存不可变草稿</button>
        </form>
      </section>
      ...
      <section className="settings-card settings-card-full">
        ...
        <div className="profile-table">{(data?.profiles ?? []).map((profile) => <div key={profile.id}><span className={`profile-state state-${profile.state.toLowerCase()}`}>{profile.state}</span><b>{profile.agent}</b><code>{profile.id}</code><small>{profile.scope}:{profile.scopeId}</small>{profile.scope === "tenant" && ["DRAFT", "DEGRADED"].includes(profile.state) ? <button disabled={busy || readOnly} onClick={() => validateProfile(profile.id)} type="button">运行探针</button> : <span />}</div>)}</div>
      </section>
    </div>
  </AppShell>;
}
