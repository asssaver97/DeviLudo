...
function OverviewTab({ catalog, versions, installations, defaultAgent, localAgents, localHealth, canChangeDefault, onDefaultChange, onNavigate }: {
  ...
  const executableProfileBindings = providerBindings.filter((binding) => binding.state === "VERIFIED" && binding.runtimeState === "READY").length;
  return (
    <>
      <div className={styles.metricRail}>
        ...
        <div><span>Gateway / Provider</span><strong>{localHealth?.dependencies?.inferenceGateway === "CONFIGURED" ? "已配置" : "未配置"}</strong><small>{verifiedProviderBindings} / {providerBindings.length} 个主或 fallback 绑定已验证</small></div>
        <div><span>开发 Worker</span><strong>{workerReady ? "READY" : "BLOCKED"}</strong><small>{providerBindings.length ? `${executableProfileBindings} / ${providerBindings.length} 条精确 Profile 路径可执行` : localHealth?.dependencies?.workerIdentityMode === "LOCAL_DETERMINISTIC" ? "本机安装可校验，等待 Provider 与执行授权" : "等待版本、镜像与 Gateway"}</small></div>
      </div>
  ...
  return (
    <div className={styles.providerLayout}>
      <section className={styles.section}>
        ...
        {localHealth?.dependencies?.agentProfileExecution === "PARTIAL" ? <div className={styles.permissionNotice}><AdminIcon name="shield" />只有部分主 Profile 或项目授权 fallback 同时满足精确 CLI 与 Provider 门禁；不可执行路径不会被其他 Agent 的健康状态掩盖。</div> : null}
        ...
        <div className={styles.providerRows}>
          {activeProviders.map(({ kind, provider }) => {
            ...
            const executable = bindings.filter((binding) => binding.state === "VERIFIED" && binding.runtimeState === "READY").length;
            const fallbacks = bindings.filter((binding) => binding.selectionRole !== "PRIMARY").length;
            const localState = bindings.length === 0 ? "NOT SELECTED"
              : executable === bindings.length ? "LOCAL READY"
                : executable > 0 ? "PARTIAL" : verified > 0 ? "CLI BLOCKED" : "RESTORE";
            const localTone = localState === "LOCAL READY" ? "success" : localState === "PARTIAL" ? "info" : "warning";
            return <button type="button" key={kind}
              ...
              <AgentMark kind={kind} small /><div><strong>{kind === "claude-code" ? "Anthropic Messages" : "OpenAI Responses"} · {provider ? providerHost(provider.baseUrl) : "未配置"}</strong>
                <span>{provider ? `${provider.protocol} · ${provider.primaryModel}` : "尚无 ACTIVE 平台 Provider"}{production ? "" : ` · Provider ${verified}/${bindings.length} · 执行 ${executable}/${bindings.length}${fallbacks ? ` · fallback ${fallbacks}` : ""}`}</span></div>
  ...
  return (
    <>
      <div className={styles.healthBanner}>
        ...
        <dl><div><dt>精确 CLI</dt><dd>{readyAgents} / 2</dd></div><div><dt>可执行 Profile</dt><dd>{executableProfileBindings} / {providerBindings.length}</dd></div><div><dt>运营告警</dt><dd>{alerts.length}</dd></div><div><dt>开发 Worker</dt><dd>{workerReady ? "READY" : "BLOCKED"}</dd></div></dl>
      </div>
      <div className={styles.metricRail}>
