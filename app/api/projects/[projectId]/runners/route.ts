...
export async function GET(
  ...
  try {
    ...
    try {
      ...
      return json({
        error: {
          code: missing ? "RUNNER_FLEET_NOT_FOUND" : "RUNNER_FLEET_UNAVAILABLE",
          message: missing ? "项目不存在或当前账号无权查看。" : "Runner Fleet 投影暂不可用。",
        },
...
function brokerRequired(): Response {
  return json({
    error: {
      code: "RUNNER_FLEET_PROJECTION_BROKER_REQUIRED",
      message: "生产 Runner 状态需要租户隔离的只读投影 Broker。",
    },
  }, { status: 503 });
}
