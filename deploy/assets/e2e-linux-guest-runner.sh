...
ssh "${ssh_options[@]}" "deviludo-guest@$ip" env \
    ...
    DEVILUDO_GUEST_EVIDENCE_ROOT=/var/lib/deviludo/output DEVILUDO_GUEST_JOB_ROOT=/var/lib/deviludo/jobs \
...
while IFS= read -r line <&4; do
  ...
  if [[ $message_type == policy_request ]]; then
    ...
    printf '%s\n' "$response" >&3
  elif [[ $message_type == result ]]; then receipt=$(jq -ce '.value' <<<"$line")
  else echo 'guest emitted an unknown frame' >&2; exit 1
  fi
done
wait "$guest_pid"
...
if [[ -n $guest_regression ]]; then
  ...
  [[ $guest_regression == /var/lib/deviludo/* ]] || { echo 'guest regression path escaped the guest job directory' >&2; exit 1; }
