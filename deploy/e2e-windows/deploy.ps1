...
function Verify-Manifest($c,$stage){
  ...
  if($manifest.schemaVersion -ne 'deviludo.release.v1' -or $manifest.version -ne $c.releaseVersion -or $manifest.roles -notcontains 'E2E_WINDOWS' -or $manifest.plugins.GODOT.version -ne '1' -or $manifest.plugins.GODOT.guestReportProtocol -ne 'deviludo.godot-guest-report.v1' -or $manifest.plugins.GODOT.artifactHostCommandsAllowed -ne $false -or $manifest.plugins.GODOT.builderImage -notmatch '@sha256:[0-9a-f]{64}$'){throw 'Release manifest is invalid'}
  return $manifest
}
