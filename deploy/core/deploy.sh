...
role_validate_config() { role_preflight; [[ ${DEVILUDO_ACCESS_MODE:-} =~ ^(standalone|platform)$ && ${DEVILUDO_PROJECTS_ROOT:-} == /var/lib/deviludo-projects && ${DEVILUDO_S3_ENDPOINT:-} == https://* && ${DEVILUDO_S3_PUBLIC_ENDPOINT:-} == https://* && ${DEVILUDO_VAULT_ADDR:-} == https://* && ${DEVILUDO_OTEL_ENDPOINT:-} == https://* && ${DEVILUDO_PROVIDER_ALLOWLIST:-} =~ ^[A-Za-z0-9.-]+(,[A-Za-z0-9.-]+)*$ && ${DEVILUDO_STEAM_ALLOWLIST:-} =~ ^[A-Za-z0-9.-]+(,[A-Za-z0-9.-]+)*$ && ${DEVILUDO_CORE_BIND_ADDRESS:-} =~ ^([0-9]{1,3}\.){3}[0-9]{1,3}$ && ${DEVILUDO_CORE_BIND_ADDRESS:-} != 0.0.0.0 && ${DEVILUDO_CORE_BIND_ADDRESS:-} != 127.0.0.1 && ${DEVILUDO_CORE_ALLOWED_CIDRS:-} =~ ^([0-9]{1,3}\.){3}[0-9]{1,3}/[0-9]{1,2}(,([0-9]{1,3}\.){3}[0-9]{1,3}/[0-9]{1,2})*$ ]] || return 1; [[ $DEVILUDO_ACCESS_MODE != platform || ${DEVILUDO_PLATFORM_ACCOUNT_API_URL:-} == https://* ]] || return 1; }
