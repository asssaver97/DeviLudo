...
role_validate_config() { role_preflight; [[ ${DEVILUDO_S3_ENDPOINT:-} == https://* && ${DEVILUDO_S3_PUBLIC_ENDPOINT:-} == https://* && ${DEVILUDO_VAULT_ADDR:-} == https://* && ${DEVILUDO_OTEL_ENDPOINT:-} == https://* && ${DEVILUDO_PROVIDER_ALLOWLIST:-} =~ ^[A-Za-z0-9.-]+(,[A-Za-z0-9.-]+)*$ && ${DEVILUDO_STEAM_ALLOWLIST:-} =~ ^[A-Za-z0-9.-]+(,[A-Za-z0-9.-]+)*$ && ${DEVILUDO_CORE_BIND_ADDRESS:-} =~ ^([0-9]{1,3}\.){3}[0-9]{1,3}$ && ${DEVILUDO_CORE_BIND_ADDRESS:-} != 0.0.0.0 && ${DEVILUDO_CORE_BIND_ADDRESS:-} != 127.0.0.1 && ${DEVILUDO_CORE_ALLOWED_CIDRS:-} =~ ^([0-9]{1,3}\.){3}[0-9]{1,3}/[0-9]{1,2}(,([0-9]{1,3}\.){3}[0-9]{1,3}/[0-9]{1,2})*$ && ${DEVILUDO_STEAM_APP_ID:-} =~ ^[0-9]{2,12}$ && ${DEVILUDO_STEAM_DEPOT_LINUX:-} =~ ^[0-9]{2,12}$ && ${DEVILUDO_STEAM_DEPOT_WINDOWS:-} =~ ^[0-9]{2,12}$ && ${DEVILUDO_STEAM_DEPOT_MACOS:-} =~ ^[0-9]{2,12}$ ]] || return 1; }
