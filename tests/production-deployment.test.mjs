...
test("every shared-image workload is rendered as a least-authority deployment and only network servers get Services", () => {
  ...
  assert.equal(classification.networked.length, 31);
  ...
  assert.equal(bundle.stages[2].resources.filter((resource) => resource.kind === "Deployment").length, 33);
  assert.equal(bundle.stages[2].resources.filter((resource) => resource.kind === "Service").length, 31);
