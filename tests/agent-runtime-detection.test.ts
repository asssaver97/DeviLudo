...
import { codexAuthenticationMethod, detectAgentRuntimes, parseRuntimeVersion } from "@/services/core/src/agent-runtime-detection";
...
test("Codex authentication exposes only the official login method", () => {
  assert.equal(codexAuthenticationMethod("Logged in using ChatGPT"), "CHATGPT");
  assert.equal(codexAuthenticationMethod("Logged in using an API key"), "API_KEY");
  assert.equal(codexAuthenticationMethod("not logged in"), "SIGNED_OUT");
});
...
test("runtime detection reports installed versions without exposing executable paths", async () => {
  ...
  assert.deepEqual(detected, [
    { kind: "CLAUDE_CODE", installed: true, version: "2.1.201", scope: "LOCAL_HOST", authentication: null },
    { kind: "CODEX_CLI", installed: false, version: null, scope: "LOCAL_HOST", authentication: null },
  ]);
});
...
test("runtime detection probes the Core environment when no local override exists", async () => {
  const detected = await detectAgentRuntimes({}, async command => command === "codex"
    ...
    : null, async () => "Logged in using ChatGPT");
  assert.deepEqual(detected, [
    { kind: "CLAUDE_CODE", installed: false, version: null, scope: "CORE_RUNTIME", authentication: null },
    { kind: "CODEX_CLI", installed: true, version: "0.146.0-alpha.3.1", scope: "CORE_RUNTIME", authentication: "CHATGPT" },
  ]);
});
