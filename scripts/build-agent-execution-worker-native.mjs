...
const ARTIFACT_FILE = "deviludo-agent-execution-worker-native.mjs";
const RECEIPT_FILE = "agent-execution-worker-native-build-receipt.json";

export function parseAgentExecutionWorkerNativeBuildArguments(argv) {
...
export function validateAgentExecutionWorkerNativeBuildReceipt(value) {
  if (!plainRecord(value) || !exactKeys(value, [
  ...
  ]) || value.schemaVersion !== "deviludo.agent-execution-worker-native-build-receipt.v1" || value.status !== "CANDIDATE"
    ...
    || value.entryPoint !== "services/agent-execution-broker/src/run-native-worker-bundle.ts"
...
export async function buildAgentExecutionWorkerNative(options, {
  ...
  const temporary = await mkdtemp(resolve(outputParent, `.agent-execution-worker-native-${uuid()}-`));
  let published = false;
  try {
    const artifactPath = resolve(temporary, ARTIFACT_FILE);
    const result = await bundle({
      entryPoints: [resolve(root, "services/agent-execution-broker/src/run-native-worker-bundle.ts")],
      ...
      format: "esm",
      packages: "bundle",
      banner: { js: "#!/usr/bin/node\nimport { createRequire as __deviludoCreateRequire } from 'node:module'; const require = __deviludoCreateRequire(import.meta.url); globalThis.__DEVILUDO_AGENT_EXECUTION_WORKER_BUNDLE__ = true;" },
      legalComments: "none",
      sourcemap: false,
      minify: false,
      metafile: true,
    ...
    const receipt = validateAgentExecutionWorkerNativeBuildReceipt({
      schemaVersion: "deviludo.agent-execution-worker-native-build-receipt.v1",
      status: "CANDIDATE",
...
function invalid() { throw new Error("Agent execution Worker native build input is invalid"); }
function invalidReceipt() { throw new Error("Agent execution Worker native build receipt is invalid"); }

async function main() {
  const options = parseAgentExecutionWorkerNativeBuildArguments(process.argv.slice(2));
  const result = await buildAgentExecutionWorkerNative(options);
  process.stdout.write(`${JSON.stringify(result)}\n`);
}
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  main().catch(() => { process.stderr.write("[build:agent-execution-worker-native] build failed\n"); process.exitCode = 1; });
}
