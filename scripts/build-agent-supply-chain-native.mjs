...
const ARTIFACT_FILE = "deviludo-agent-supply-chain-native.mjs";
const RECEIPT_FILE = "agent-supply-chain-native-build-receipt.json";

export function parseAgentSupplyChainNativeBuildArguments(argv) {
...
export function validateAgentSupplyChainNativeBuildReceipt(value) {
  ...
  ]) || value.schemaVersion !== "deviludo.agent-supply-chain-native-build-receipt.v2" || value.status !== "CANDIDATE"
    || typeof value.platformVersion !== "string" || !fixedVersion(value.platformVersion)
    || typeof value.sourceRevision !== "string" || !SOURCE_REVISION.test(value.sourceRevision)
    || value.nodeTarget !== "22.13" || !digest(value.packageLockDigest) || value.esbuildVersion !== "0.28.0"
    || !digest(value.esbuildLibraryDigest) || value.entryPoint !== "services/agent-supply-chain/src/run-native-policy.ts"
...
export async function buildAgentSupplyChainNative(options, {
  root = resolve("."),
  bundle = esbuild,
  now = () => new Date(),
  verifySource = verifyGitSource,
  uuid = randomUUID,
} = {}) {
  ...
  const esbuildLibraryPath = resolve(root, "node_modules/esbuild/lib/main.js");
  const [packageJsonBytes, packageLockBytes, esbuildLibraryBytes] = await Promise.all([
    readFile(packageJsonPath), readFile(packageLockPath), readFile(esbuildLibraryPath),
  ]);
  let packageJson;
  let packageLock;
  try {
  ...
  const temporary = await mkdtemp(resolve(outputParent, `.agent-supply-chain-native-${uuid()}-`));
  let published = false;
  try {
    const artifactPath = resolve(temporary, ARTIFACT_FILE);
    const result = await bundle({
      entryPoints: [resolve(root, "services/agent-supply-chain/src/run-native-policy.ts")],
    ...
    const receipt = validateAgentSupplyChainNativeBuildReceipt({
      ...
      bundleInputDigest: sha256Canonical(inputs),
...
function sha256Canonical(value) { return hash(Buffer.from(JSON.stringify(canonicalize(value)))); }
function canonicalize(value) {
  if (Array.isArray(value)) return value.map(canonicalize);
  if (plainRecord(value)) return Object.fromEntries(Object.keys(value).sort().map((key) => [key, canonicalize(value[key])]));
  return value;
}
function digest(value) { return typeof value === "string" && /^[a-f0-9]{64}$/.test(value); }
function fixedVersion(value) { return VERSION.test(value) && !/(?:latest|stable|default)/i.test(value); }
...
function invalid() { throw new Error("Agent supply-chain native build input is invalid"); }
function invalidReceipt() { throw new Error("Agent supply-chain native build receipt is invalid"); }

async function main() {
  const options = parseAgentSupplyChainNativeBuildArguments(process.argv.slice(2));
  const result = await buildAgentSupplyChainNative(options);
  const { outputDirectory, ...receipt } = result;
  process.stdout.write(`${JSON.stringify({ ...receipt, outputDirectory })}\n`);
}

if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  main().catch(() => {
    process.stderr.write("[build:agent-supply-chain-native] build failed\n");
    process.exitCode = 1;
  });
}
