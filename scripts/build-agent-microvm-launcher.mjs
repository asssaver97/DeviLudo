...
const ARTIFACT_FILE = "deviludo-agent-microvm-launcher.mjs";
const RECEIPT_FILE = "agent-microvm-launcher-build-receipt.json";

export function parseAgentMicrovmLauncherBuildArguments(argv) {
  if (!Array.isArray(argv) || argv.length !== 4) invalid();
  const values = new Map();
  for (let index = 0; index < argv.length; index += 2) {
    const name = argv[index]; const value = argv[index + 1];
    if (!new Set(["--source-revision", "--output-directory"]).has(name) || typeof value !== "string" || !value
      || values.has(name) || /[\0\r\n]/.test(value)) invalid();
    values.set(name, value);
  }
  const sourceRevision = values.get("--source-revision"); const outputDirectory = values.get("--output-directory");
...
export function validateAgentMicrovmLauncherBuildReceipt(value) {
  if (!plainRecord(value) || !exactKeys(value, [
  ...
  ]) || value.schemaVersion !== "deviludo.agent-microvm-launcher-build-receipt.v1" || value.status !== "CANDIDATE"
    ...
    || value.entryPoint !== "services/agent-execution-broker/src/native-microvm-launcher.ts"
...
export async function buildAgentMicrovmLauncher(options, {
  ...
  try { packageJson = JSON.parse(packageJsonBytes); packageLock = JSON.parse(packageLockBytes); } catch { invalid(); }
  validateLockedBuild(packageJson, packageLock);
  const temporary = await mkdtemp(resolve(dirname(options.outputDirectory), `.agent-microvm-launcher-${uuid()}-`));
  let published = false;
  try {
    const artifactPath = resolve(temporary, ARTIFACT_FILE);
    const result = await bundle({
      entryPoints: [resolve(root, "services/agent-execution-broker/src/native-microvm-launcher.ts")],
      outfile: artifactPath, bundle: true, platform: "node", target: "node22.13", format: "esm", packages: "bundle",
      banner: { js: "#!/usr/bin/node" }, legalComments: "none", sourcemap: false, minify: false, metafile: true,
      logLevel: "warning",
    });
    await chmod(artifactPath, 0o500);
    const artifact = await readFile(artifactPath); const inputs = Object.keys(result.metafile?.inputs ?? {}).sort();
    const completedAt = now();
    if (!(completedAt instanceof Date) || !Number.isFinite(completedAt.getTime()) || inputs.length < 1) invalid();
    const receipt = validateAgentMicrovmLauncherBuildReceipt({
      schemaVersion: "deviludo.agent-microvm-launcher-build-receipt.v1", status: "CANDIDATE",
      platformVersion: packageJson.version, sourceRevision: options.sourceRevision, nodeTarget: "22.13",
      packageLockDigest: hash(packageLockBytes), esbuildVersion: "0.28.0", esbuildLibraryDigest: hash(esbuildLibraryBytes),
      entryPoint: "services/agent-execution-broker/src/native-microvm-launcher.ts", artifactFileName: ARTIFACT_FILE,
      artifactDigest: hash(artifact), sizeBytes: artifact.byteLength, bundleInputCount: inputs.length,
      bundleInputDigest: hash(Buffer.from(JSON.stringify(inputs))), completedAt: completedAt.toISOString(),
    });
    await writeFile(resolve(temporary, RECEIPT_FILE), `${JSON.stringify(receipt, null, 2)}\n`, { flag: "wx", mode: 0o400 });
    await rename(temporary, options.outputDirectory); published = true;
...
async function verifyGitSource(root, sourceRevision) {
  const [head, status] = await Promise.all([capture("git", ["-C", root, "rev-parse", "HEAD"]),
    capture("git", ["-C", root, "status", "--porcelain", "--untracked-files=normal"])]);
  if (head.trim() !== sourceRevision || status.trim()) invalid();
}
function capture(command, args) { return new Promise((accept, reject) => execFile(command, args, {
...
function validateLockedBuild(packageJson, packageLock) {
  ...
  if (!plainRecord(packageJson) || !fixedVersion(packageJson.version) || packageJson.devDependencies?.esbuild !== "0.28.0"
    ...
    || locked.resolved !== "https://registry.npmjs.org/esbuild/-/esbuild-0.28.0.tgz" || locked.integrity !== ESBUILD_INTEGRITY) invalid();
}
...
function absolute(value) { return typeof value === "string" && isAbsolute(value) && resolve(value) === value && value.length <= 4096; }
...
function invalid() { throw new Error("Agent microVM launcher build input is invalid"); }
function invalidReceipt() { throw new Error("Agent microVM launcher build receipt is invalid"); }

async function main() {
  const result = await buildAgentMicrovmLauncher(parseAgentMicrovmLauncherBuildArguments(process.argv.slice(2)));
  process.stdout.write(`${JSON.stringify(result)}\n`);
}
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  main().catch(() => { process.stderr.write("[build:agent-microvm-launcher] build failed\n"); process.exitCode = 1; });
}
