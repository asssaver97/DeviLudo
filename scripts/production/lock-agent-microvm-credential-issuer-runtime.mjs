...
export function parseAgentMicrovmCredentialIssuerRuntimeLockArguments(argv) {
...
export async function createAgentMicrovmCredentialIssuerRuntimeLock({
  clusterContext,
  configurationRevision,
  createdAt = new Date(),
  lockId = randomUUID(),
  namespace = "deviludo-agent-credentials",
} = {}, inspect = inspectAgentMicrovmCredentialIssuerRuntimeResources) {
  const skeleton = runtimeLockSkeleton({ clusterContext, configurationRevision, createdAt, lockId, namespace });
  const expected = agentMicrovmCredentialIssuerRuntimeLockResources(skeleton);
  ...
  return validateAgentMicrovmCredentialIssuerRuntimeLock({
    ...skeleton,
...
export function validateAgentMicrovmCredentialIssuerRuntimeLock(lock, expected = {}) {
  if (!plainRecord(lock) || !exactKeys(lock, LOCK_KEYS)
    || lock.schemaVersion !== "deviludo.agent-microvm-credential-issuer-runtime-lock.v1"
  ...
  validateLockedResource(lock.registrySecret, "Secret", `deviludo-agent-credential-registry-${revision}`);
  validateLockedResource(lock.configMap, "ConfigMap", `deviludo-agent-credential-config-${revision}`);
  validateLockedResource(lock.environmentSecret, "Secret", `deviludo-agent-credential-environment-${revision}`);
  validateLockedResource(lock.filesSecret, "Secret", `deviludo-agent-credential-files-${revision}`);
  const resources = agentMicrovmCredentialIssuerRuntimeLockResources(lock);
...
export async function verifyAgentMicrovmCredentialIssuerRuntimeLock(lock, expected,
  inspect = inspectAgentMicrovmCredentialIssuerRuntimeResources) {
  const trusted = validateAgentMicrovmCredentialIssuerRuntimeLock(lock, expected);
  const resources = agentMicrovmCredentialIssuerRuntimeLockResources(trusted);
  const observed = normalizeObservedResources(await inspect(Object.freeze({
  ...
  return Object.freeze({
    ...
    runtimeLockDigest: agentMicrovmCredentialIssuerRuntimeLockDigest(trusted),
    resourceCount: resources.length,
  });
}

export function agentMicrovmCredentialIssuerRuntimeLockDigest(lock) {
  return `sha256:${createHash("sha256").update(canonicalJson(validateAgentMicrovmCredentialIssuerRuntimeLock(lock))).digest("hex")}`;
}

export function agentMicrovmCredentialIssuerRuntimeLockResources(lock) {
...
export async function inspectAgentMicrovmCredentialIssuerRuntimeResources(request, execute = executeKubectlCapture) {
  ...
  return parseAgentMicrovmCredentialIssuerRuntimeMetadataOutput(output);
}

export function parseAgentMicrovmCredentialIssuerRuntimeMetadataOutput(output) {
...
function runtimeLockSkeleton({ clusterContext, configurationRevision, createdAt, lockId, namespace }) {
  ...
  return {
    ...
    registrySecret: { kind: "Secret", name: `deviludo-agent-credential-registry-${configurationRevision}` },
    configMap: { kind: "ConfigMap", name: `deviludo-agent-credential-config-${configurationRevision}` },
    environmentSecret: { kind: "Secret", name: `deviludo-agent-credential-environment-${configurationRevision}` },
    filesSecret: { kind: "Secret", name: `deviludo-agent-credential-files-${configurationRevision}` },
  };
}
...
async function executeKubectlCapture(invocation) { return new Promise((accept, reject) => {
  ...
  child.once("error", reject); child.once("exit", (code, signal) => code === 0 && signal === null && length <= MAX_OUTPUT_BYTES
    ...
    : reject(new Error("Agent microVM credential issuer Kubernetes runtime metadata probe failed")));
}); }
function invalidInput() { throw new Error("Agent microVM credential issuer runtime lock input is invalid"); }
function invalidLock() { throw new Error("Agent microVM credential issuer runtime lock is invalid"); }

async function main() {
  if (process.env.NODE_ENV !== "production") invalidInput();
  const lock = await createAgentMicrovmCredentialIssuerRuntimeLock(
    parseAgentMicrovmCredentialIssuerRuntimeLockArguments(process.argv.slice(2)));
  process.stdout.write(`${JSON.stringify(lock)}\n`);
}

if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  main().catch(() => { process.stderr.write("[lock:agent-microvm-credential-issuer-runtime] lock creation failed\n"); process.exitCode = 1; });
}
