...
export function parseSteamWorkflowExecutorRuntimeLockArguments(argv) {
...
export async function createSteamWorkflowExecutorRuntimeLock({
  clusterContext,
  configurationRevision,
  createdAt = new Date(),
  lockId = randomUUID(),
  namespace = "deviludo-steam-release",
} = {}, inspect = inspectSteamWorkflowExecutorRuntimeResources) {
  const skeleton = runtimeLockSkeleton({ clusterContext, configurationRevision, createdAt, lockId, namespace });
  const expected = steamWorkflowExecutorRuntimeLockResources(skeleton);
  ...
  return validateSteamWorkflowExecutorRuntimeLock({
    ...skeleton,
...
export function validateSteamWorkflowExecutorRuntimeLock(lock, expected = {}) {
  if (!plainRecord(lock) || !exactKeys(lock, LOCK_KEYS)
    || lock.schemaVersion !== "deviludo.steam-workflow-executor-runtime-lock.v1"
    || !UUID_V4.test(lock.lockId) || !CONTEXT.test(lock.clusterContext) || !REVISION.test(lock.configurationRevision)
    || !validNamespace(lock.namespace) || !canonicalTimestamp(lock.createdAt)) invalidLock();
  const revision = lock.configurationRevision;
  validateLockedResource(lock.registrySecret, "Secret", `deviludo-steam-workflow-executor-registry-${revision}`);
  validateLockedResource(lock.configMap, "ConfigMap", `deviludo-steam-workflow-executor-config-${revision}`);
  validateLockedResource(lock.environmentSecret, "Secret", `deviludo-steam-workflow-executor-environment-${revision}`);
  validateLockedResource(lock.filesSecret, "Secret", `deviludo-steam-workflow-executor-files-${revision}`);
  const resources = steamWorkflowExecutorRuntimeLockResources(lock);
  ...
  if (expected.configurationRevision !== undefined && expected.configurationRevision !== lock.configurationRevision) invalidLock();
  return freezeLock(lock);
}

export async function verifySteamWorkflowExecutorRuntimeLock(lock, expected,
  inspect = inspectSteamWorkflowExecutorRuntimeResources) {
  const trusted = validateSteamWorkflowExecutorRuntimeLock(lock, expected);
  const resources = steamWorkflowExecutorRuntimeLockResources(trusted);
  const observed = normalizeObservedResources(await inspect(Object.freeze({
  ...
  return Object.freeze({
    ...
    runtimeLockDigest: steamWorkflowExecutorRuntimeLockDigest(trusted),
    resourceCount: resources.length,
  });
}

export function steamWorkflowExecutorRuntimeLockDigest(lock) {
  return `sha256:${createHash("sha256").update(canonicalJson(validateSteamWorkflowExecutorRuntimeLock(lock))).digest("hex")}`;
}

export function steamWorkflowExecutorRuntimeLockResources(lock) {
...
export async function inspectSteamWorkflowExecutorRuntimeResources(request, execute = executeKubectlCapture) {
  ...
  return parseSteamWorkflowExecutorRuntimeMetadataOutput(output);
}

export function parseSteamWorkflowExecutorRuntimeMetadataOutput(output) {
  ...
  return Object.freeze(rows.map(([kind, name, uid, resourceVersion, immutable]) => validateObservedResource({
    kind, name, uid, resourceVersion, immutable: immutable === "true",
  })));
}
...
function runtimeLockSkeleton({ clusterContext, configurationRevision, createdAt, lockId, namespace }) {
  ...
  return {
    ...
    registrySecret: { kind: "Secret", name: `deviludo-steam-workflow-executor-registry-${configurationRevision}` },
    configMap: { kind: "ConfigMap", name: `deviludo-steam-workflow-executor-config-${configurationRevision}` },
    environmentSecret: { kind: "Secret", name: `deviludo-steam-workflow-executor-environment-${configurationRevision}` },
    filesSecret: { kind: "Secret", name: `deviludo-steam-workflow-executor-files-${configurationRevision}` },
  };
}
...
function validateLockedResource(resource, kind, name) {
  if (!plainRecord(resource) || !exactKeys(resource, RESOURCE_KEYS) || resource.kind !== kind || resource.name !== name
    || !KUBERNETES_UID.test(resource.uid) || !RESOURCE_VERSION.test(resource.resourceVersion)) invalidLock();
}
...
async function executeKubectlCapture(invocation) { return new Promise((accept, reject) => {
  ...
  child.once("error", reject); child.once("exit", (code, signal) => code === 0 && signal === null && length <= MAX_OUTPUT_BYTES
    ...
    : reject(new Error("Steam workflow executor Kubernetes runtime metadata probe failed")));
}); }
function invalidInput() { throw new Error("Steam workflow executor runtime lock input is invalid"); }
function invalidLock() { throw new Error("Steam workflow executor runtime lock is invalid"); }

async function main() {
  if (process.env.NODE_ENV !== "production") invalidInput();
  const lock = await createSteamWorkflowExecutorRuntimeLock(
    parseSteamWorkflowExecutorRuntimeLockArguments(process.argv.slice(2)),
  );
  process.stdout.write(`${JSON.stringify(lock)}\n`);
}
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  main().catch(() => { process.stderr.write("[lock:steam-workflow-executor-runtime] lock creation failed\n"); process.exitCode = 1; });
}
