...
import { validateAgentExecutionWorkerNativeBuildReceipt } from "../build-agent-execution-worker-native.mjs";
...
export function parseAgentExecutionWorkerNativeFinalizationArguments(argv) {
  ...
  if (!SOURCE_REVISION.test(values.get("--source-revision")) || !UUID.test(values.get("--release-id"))
    || !canonicalTimestamp(values.get("--published-at")) || !SHA256.test(values.get("--trust-policy-digest"))) invalidInput();
...
export async function prepareAgentExecutionWorkerNativeClaims(options) {
  ...
  const build = validateAgentExecutionWorkerNativeBuildReceipt(parseJson(buildBytes));
  ...
  return Object.freeze({
    kind: "deviludo-agent-execution-worker-native",
    version: 1,
...
export class MtlsAgentExecutionWorkerNativeSigner {
  ...
  async sign(claims, trustPolicy, trustPolicyDigest, now = new Date()) {
    ...
    const trusted = validateAgentExecutionWorkerNativeTrustPolicy(trustPolicy, trustPolicyDigest);
    ...
    const response = await this.request(Object.freeze({
      url: new URL("/v1/agent-execution-worker-native/sign-ed25519", this.endpoint),
      tls: this.tls,
      headers: Object.freeze({ "content-type": "application/json", "idempotency-key": claimsDigest }),
      body: JSON.stringify({
        schemaVersion: "deviludo.agent-execution-worker-native-signing-request.v1",
        keyId: this.keyId,
        claimsDigest,
    ...
    if (response.statusCode !== 200 || !plainRecord(response.body) || !exactKeys(response.body, RESPONSE_KEYS)
      || response.body.schemaVersion !== "deviludo.agent-execution-worker-native-signing-response.v1"
...
export async function agentExecutionWorkerNativeSignerFromEnvironment(env = process.env) {
  const [key, cert, ca] = await Promise.all([
    tlsFile(env.DEVILUDO_AGENT_EXECUTION_WORKER_NATIVE_SIGNER_TLS_KEY_FILE),
    tlsFile(env.DEVILUDO_AGENT_EXECUTION_WORKER_NATIVE_SIGNER_TLS_CERT_FILE),
    tlsFile(env.DEVILUDO_AGENT_EXECUTION_WORKER_NATIVE_SIGNER_TLS_CA_FILE),
  ]);
  return new MtlsAgentExecutionWorkerNativeSigner({
    endpoint: env.DEVILUDO_AGENT_EXECUTION_WORKER_NATIVE_SIGNER_ENDPOINT,
    keyId: env.DEVILUDO_AGENT_EXECUTION_WORKER_NATIVE_SIGNING_KEY_ID,
    tls: { key, cert, ca },
  });
}

export async function finalizeAgentExecutionWorkerNative(options, { signer, now = new Date() } = {}) {
  ...
  const [claims, trustPolicy] = await Promise.all([
    prepareAgentExecutionWorkerNativeClaims(options), readBoundedJson(options.trustPolicyPath),
  ]);
...
function verifyManifest(manifest, claims, trustPolicy, trustPolicyDigest, now) {
  return verifySignedAgentExecutionWorkerNativeRelease(manifest, {
...
function validateEvidence(value, artifactDigest, buildReceiptDigest) {
  if (!plainRecord(value) || !exactKeys(value, EVIDENCE_KEYS)
    || value.schemaVersion !== "deviludo.agent-execution-worker-native-evidence.v1" || value.scanState !== "PASS"
...
async function boundedFile(path, maximum) {
  ...
  try {
    ...
    if (!before.isFile() || before.size < 1 || before.size > maximum || (before.mode & 0o022) !== 0) invalidInput();
    const value = await file.readFile(); const after = await file.stat();
    if (value.byteLength !== before.size || after.size !== before.size || after.mtimeMs !== before.mtimeMs) invalidInput();
    return value;
  } finally { await file.close(); }
}
async function readBoundedJson(path) { return parseJson(await boundedFile(path, MAX_JSON_BYTES)); }
async function readJsonIfPresent(path) {
  try { return await readBoundedJson(path); } catch (error) { if (error?.code === "ENOENT") return null; throw error; }
}
async function tlsFile(path) {
  if (!absoluteValue(path)) invalidInput();
  const value = await boundedFile(path, MAX_TLS_BYTES);
  if (value.length < 32 || value.includes(0)) invalidInput(); return value;
}
  ...
  return new Promise((accept, reject) => {
    const request = httpsRequest(input.url, {
      ...
      if (response.headers["content-type"] !== "application/json") {
        response.resume(); reject(new Error("Agent execution Worker native signing response is invalid")); return;
      }
      const chunks = []; let length = 0;
      response.on("data", (chunk) => {
        length += chunk.length;
        if (length > MAX_RESPONSE_BYTES) request.destroy(new Error("Agent execution Worker native signing response is invalid"));
        else chunks.push(chunk);
      });
      response.on("end", () => {
        try { accept(Object.freeze({ statusCode: response.statusCode, body: JSON.parse(Buffer.concat(chunks).toString("utf8")) })); }
        catch { reject(new Error("Agent execution Worker native signing response is invalid")); }
      });
    });
    request.on("timeout", () => request.destroy(new Error("Agent execution Worker native signing request timed out")));
    request.on("error", reject); request.end(input.body);
  });
}
function signerEndpoint(value) {
  let url; try { url = new URL(value); } catch { invalidInput(); }
...
function invalidInput() { throw new Error("Agent execution Worker native finalization input is invalid"); }

async function main() {
  if (process.env.NODE_ENV !== "production") invalidInput();
  const options = parseAgentExecutionWorkerNativeFinalizationArguments(process.argv.slice(2));
  const signer = await agentExecutionWorkerNativeSignerFromEnvironment();
  const result = await finalizeAgentExecutionWorkerNative(options, { signer });
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  main().catch(() => { process.stderr.write("[finalize:agent-execution-worker-native] finalization failed\n"); process.exitCode = 1; });
}
