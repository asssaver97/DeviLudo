...
export function parseWindowsScmNativeActuatorFinalizationArguments(argv) {
  if (!Array.isArray(argv) || argv.length !== 20) invalidInput();
  const allowed = new Set([
    "--actuator-version", "--architecture", "--binary", "--built-at", "--evidence", "--output", "--revision",
  ...
  if (!new Set(["x86_64", "arm64"]).has(values.get("--architecture"))
    || !fixedVersion(values.get("--actuator-version")) || !canonicalTimestamp(values.get("--built-at"))
  ...
  return Object.freeze({
    actuatorVersion: values.get("--actuator-version"),
...
export async function prepareWindowsScmNativeActuatorClaims(options) {
  ...
  return Object.freeze({
    kind: "deviludo-windows-scm-native-actuator",
    version: 1,
    revision: options.revision,
    platform: "windows",
    architecture: options.architecture,
    actuatorVersion: options.actuatorVersion,
...
export class MtlsWindowsScmNativeActuatorSigner {
  ...
  async sign(claims, trustPolicy, trustPolicyDigest, now = new Date()) {
    ...
    const trusted = validateWindowsScmNativeActuatorTrustPolicy(trustPolicy, trustPolicyDigest);
    ...
    const response = await this.request(Object.freeze({
      url: new URL("/v1/windows-scm-native-actuators/sign-ed25519", this.endpoint),
      tls: this.tls,
      headers: Object.freeze({ "content-type": "application/json", "idempotency-key": claimsDigest }),
      body: JSON.stringify({
        schemaVersion: "deviludo.windows-scm-native-actuator-signing-request.v1",
        keyId: this.keyId,
        claimsDigest,
    ...
    if (response.statusCode !== 200 || !plainRecord(response.body) || !exactKeys(response.body, SIGNER_RESPONSE_KEYS)
      || response.body.schemaVersion !== "deviludo.windows-scm-native-actuator-signing-response.v1"
    ...
    verifySignedWindowsScmNativeActuatorManifest(manifest, {
...
export async function windowsScmNativeActuatorSignerFromEnvironment(env = process.env) {
  const [key, cert, ca] = await Promise.all([
    boundedTlsFile(env.DEVILUDO_WINDOWS_SCM_ACTUATOR_SIGNER_TLS_KEY_FILE),
    boundedTlsFile(env.DEVILUDO_WINDOWS_SCM_ACTUATOR_SIGNER_TLS_CERT_FILE),
    boundedTlsFile(env.DEVILUDO_WINDOWS_SCM_ACTUATOR_SIGNER_TLS_CA_FILE),
  ]);
  return new MtlsWindowsScmNativeActuatorSigner({
    endpoint: env.DEVILUDO_WINDOWS_SCM_ACTUATOR_SIGNER_ENDPOINT,
    keyId: env.DEVILUDO_WINDOWS_SCM_ACTUATOR_SIGNING_KEY_ID,
    tls: { key, cert, ca },
  });
}

export async function finalizeWindowsScmNativeActuator(options, { signer, now = new Date() } = {}) {
  ...
  const [claims, trustPolicy] = await Promise.all([
    prepareWindowsScmNativeActuatorClaims(options),
  ...
  if (replay) {
    ...
    verifySignedWindowsScmNativeActuatorManifest(replay, {
  ...
  try {
    ...
    verifySignedWindowsScmNativeActuatorManifest(existing, {
      trustPolicy, trustPolicyDigest: options.trustPolicyDigest, architecture: options.architecture, now,
    });
...
function validateSigningEvidence(evidence, architecture, binary) {
  if (!plainRecord(evidence) || !exactKeys(evidence, EVIDENCE_KEYS)
    || evidence.schemaVersion !== "deviludo.windows-scm-native-actuator-signing-evidence.v1"
...
function validateOptions(options) {
  if (!plainRecord(options) || !absoluteValue(options.binaryPath) || !absoluteValue(options.evidencePath)
    ...
    || !new Set(["x86_64", "arm64"]).has(options.architecture) || !fixedVersion(options.actuatorVersion)
  ...
  return new Promise((accept, reject) => {
    const request = httpsRequest(input.url, {
      ...
      if (response.headers["content-type"] !== "application/json") {
        response.resume(); reject(new Error("Windows SCM actuator signing response is invalid")); return;
      }
      const chunks = [];
      let length = 0;
      response.on("data", (chunk) => {
        length += chunk.length;
        if (length > MAX_RESPONSE_BYTES) request.destroy(new Error("Windows SCM actuator signing response is invalid"));
        else chunks.push(chunk);
      });
      response.on("end", () => {
        try { accept(Object.freeze({ statusCode: response.statusCode, body: JSON.parse(Buffer.concat(chunks).toString("utf8")) })); }
        catch { reject(new Error("Windows SCM actuator signing response is invalid")); }
      });
    });
    request.on("timeout", () => request.destroy(new Error("Windows SCM actuator signing request timed out")));
...
function invalidInput() { throw new Error("Windows SCM native actuator finalization input is invalid"); }

async function main() {
  if (process.env.NODE_ENV !== "production") invalidInput();
  const options = parseWindowsScmNativeActuatorFinalizationArguments(process.argv.slice(2));
  const signer = await windowsScmNativeActuatorSignerFromEnvironment();
  const result = await finalizeWindowsScmNativeActuator(options, { signer });
  process.stdout.write(`${JSON.stringify({
    schemaVersion: "deviludo.windows-scm-native-actuator-finalization-result.v1",
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  main().catch(() => {
    process.stderr.write("[finalize:windows-scm-native-actuator] finalization failed\n");
    process.exitCode = 1;
  });
}
