...
export function validateArtifactPreparerImageSpec(input, platformVersion) {
  ...
  if (!base || !base.groups?.repository.endsWith("/node") || Number(base.groups.minor) < 15
    || !destination || !destination.groups?.repository.endsWith("/artifact-preparer")
...
export function artifactPreparerImageBuildCommand(spec, metadataFile) {
  const value = validateArtifactPreparerImageSpec(spec, spec?.platformVersion);
  ...
  return Object.freeze({ command: "docker", args: Object.freeze([
    "buildx", "build",
    "--file", "Dockerfile.artifact-preparer",
...
export function parseArtifactPreparerImageBuildMetadata(value) {
  const digest = plainRecord(value) ? value["containerimage.digest"] : undefined;
  if (typeof digest !== "string" || !IMAGE_DIGEST.test(digest)) {
    throw new Error("Artifact Preparer image digest is missing from BuildKit metadata");
  }
  return digest;
}

export function artifactPreparerImageReceipt(spec, metadata, inputs, completedAt = new Date().toISOString()) {
  const value = validateArtifactPreparerImageSpec(spec, spec?.platformVersion);
  const imageDigest = parseArtifactPreparerImageBuildMetadata(metadata);
...
export function validateArtifactPreparerImageReceipt(receipt, expected) {
  const base = typeof receipt?.baseImage === "string" ? NODE_BASE.exec(receipt.baseImage) : null;
  if (!plainRecord(receipt) || !exactKeys(receipt, [
    "schemaVersion", "imageReference", "imageDigest", "baseImage", "sourceRevision", "platform",
    "platformVersion", "dockerfileDigest", "packageLockDigest", "attestations", "completedAt",
  ]) || receipt.schemaVersion !== "deviludo.artifact-preparer-image-receipt.v1"
  ...
  if (!new RegExp(`^${OCI_REPOSITORY}$`).test(reference) || !reference.endsWith("/artifact-preparer")) invalidReceipt();
  return Object.freeze({ ...receipt, attestations: Object.freeze([...receipt.attestations]) });
}

export function parseArtifactPreparerImageArguments(argv, platformVersion) {
  ...
  return validateArtifactPreparerImageSpec({
...
async function main() {
  ...
  const spec = parseArtifactPreparerImageArguments(process.argv.slice(2), packageJson.version);
  const temporary = await mkdtemp(join(tmpdir(), "deviludo-artifact-preparer-image-"));
  const metadataFile = join(temporary, "metadata.json");
  try {
    const build = artifactPreparerImageBuildCommand(spec, metadataFile);
    await run(build.command, build.args, root);
    const receipt = artifactPreparerImageReceipt(spec, JSON.parse(await readFile(metadataFile, "utf8")), {
...
function run(command, args, cwd) { return new Promise((accept, reject) => {
  ...
  child.once("error", reject); child.once("exit", (code, signal) => code === 0 && signal === null
    ? accept() : reject(new Error("Artifact Preparer image build failed")));
}); }
...
function invalid() { throw new Error("Artifact Preparer image build input is invalid"); }
function invalidReceipt() { throw new Error("Artifact Preparer image receipt is invalid"); }

if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  main().catch(() => { process.stderr.write("[image:build-artifact-preparer] build failed\n"); process.exitCode = 1; });
}
