...
const LOCK_KEYS = Object.freeze([
  ...
  "lockId", "namespace", "registrySecret", "releaseVolumeClaim", "schemaVersion",
]);
...
export function parseAgentSupplyChainRuntimeLockArguments(argv) {
...
export async function createAgentSupplyChainRuntimeLock({
  clusterContext,
  configurationRevision,
  createdAt = new Date(),
  lockId = randomUUID(),
  namespace = "deviludo-agent-supply-chain",
} = {}, inspect = inspectAgentSupplyChainRuntimeResources) {
  const skeleton = runtimeLockSkeleton({ clusterContext, configurationRevision, createdAt, lockId, namespace });
  const expected = agentSupplyChainRuntimeLockResources(skeleton);
  ...
  return validateAgentSupplyChainRuntimeLock({
    ...skeleton,
    ...
    releaseVolumeClaim: lockedResource(byName, skeleton.releaseVolumeClaim),
  });
}

export function validateAgentSupplyChainRuntimeLock(lock, expected = {}) {
  if (!plainRecord(lock) || !exactKeys(lock, LOCK_KEYS)
    || lock.schemaVersion !== "deviludo.agent-supply-chain-runtime-lock.v1"
  ...
  validateLockedResource(lock.registrySecret, "Secret", `deviludo-agent-supply-chain-registry-${revision}`);
  validateLockedResource(lock.configMap, "ConfigMap", `deviludo-agent-supply-chain-config-${revision}`);
  validateLockedResource(lock.environmentSecret, "Secret", `deviludo-agent-supply-chain-environment-${revision}`);
  validateLockedResource(lock.filesSecret, "Secret", `deviludo-agent-supply-chain-files-${revision}`);
  validateLockedResource(lock.releaseVolumeClaim, "PersistentVolumeClaim", `deviludo-agent-supply-chain-release-${revision}`);
  const resources = agentSupplyChainRuntimeLockResources(lock);
...
export async function verifyAgentSupplyChainRuntimeLock(lock, expected, inspect = inspectAgentSupplyChainRuntimeResources) {
  const trusted = validateAgentSupplyChainRuntimeLock(lock, expected);
  const resources = agentSupplyChainRuntimeLockResources(trusted);
  const observed = normalizeObservedResources(await inspect(Object.freeze({
  ...
  return Object.freeze({
    ...
    runtimeLockDigest: agentSupplyChainRuntimeLockDigest(trusted),
    resourceCount: resources.length,
  });
}

export function agentSupplyChainRuntimeLockDigest(lock) {
  return `sha256:${createHash("sha256").update(canonicalJson(validateAgentSupplyChainRuntimeLock(lock))).digest("hex")}`;
}

export function agentSupplyChainRuntimeLockResources(lock) {
  return Object.freeze([
    lock.registrySecret,
    lock.configMap,
    lock.environmentSecret,
    lock.filesSecret,
    lock.releaseVolumeClaim,
  ].map((resource) => Object.freeze({ ...resource })));
}

export async function inspectAgentSupplyChainRuntimeResources(request, execute = executeKubectlCapture) {
  if (!plainRecord(request) || typeof request.clusterContext !== "string" || !CONTEXT.test(request.clusterContext)
    || !validNamespace(request.namespace) || !Array.isArray(request.resources) || request.resources.length !== 5) invalidInput();
  const resources = request.resources.map((resource) => {
    if (!plainRecord(resource) || !new Set(["ConfigMap", "Secret", "PersistentVolumeClaim"]).has(resource.kind)
  ...
  return parseAgentSupplyChainRuntimeMetadataOutput(output);
}

export function parseAgentSupplyChainRuntimeMetadataOutput(output) {
  ...
  return Object.freeze(rows.map(([kind, name, uid, resourceVersion, immutable]) => validateObservedResource({
    ...
    uid,
    resourceVersion,
    immutable: immutable === "true" ? true : immutable === "<none>" ? null : false,
  })));
}
...
function runtimeLockSkeleton({ clusterContext, configurationRevision, createdAt, lockId, namespace }) {
  ...
  return {
    ...
    registrySecret: { kind: "Secret", name: `deviludo-agent-supply-chain-registry-${configurationRevision}` },
    configMap: { kind: "ConfigMap", name: `deviludo-agent-supply-chain-config-${configurationRevision}` },
    environmentSecret: { kind: "Secret", name: `deviludo-agent-supply-chain-environment-${configurationRevision}` },
    filesSecret: { kind: "Secret", name: `deviludo-agent-supply-chain-files-${configurationRevision}` },
    releaseVolumeClaim: {
      kind: "PersistentVolumeClaim",
      name: `deviludo-agent-supply-chain-release-${configurationRevision}`,
    },
  };
}
...
function validateObservedResource(resource) {
  if (!plainRecord(resource) || !exactKeys(resource, OBSERVED_KEYS)
    || !new Set(["ConfigMap", "Secret", "PersistentVolumeClaim"]).has(resource.kind)
    || !validResourceName(resource.name) || typeof resource.uid !== "string" || !KUBERNETES_UID.test(resource.uid)
    || typeof resource.resourceVersion !== "string" || !RESOURCE_VERSION.test(resource.resourceVersion)) invalidLock();
  if (resource.kind === "PersistentVolumeClaim" ? resource.immutable !== null : resource.immutable !== true) invalidLock();
...
function freezeLock(lock) {
  return Object.freeze({
    ...
    configMap: Object.freeze({ ...lock.configMap }),
    environmentSecret: Object.freeze({ ...lock.environmentSecret }),
    filesSecret: Object.freeze({ ...lock.filesSecret }),
    releaseVolumeClaim: Object.freeze({ ...lock.releaseVolumeClaim }),
  });
}

function validNamespace(value) { return typeof value === "string" && value.length <= 63 && NAMESPACE.test(value); }
function validResourceName(value) { return typeof value === "string" && value.length <= 253 && NAMESPACE.test(value); }
...
async function executeKubectlCapture(invocation) {
  return new Promise((accept, reject) => {
    ...
    child.stdout.on("data", (chunk) => {
      length += chunk.length;
      if (length > MAX_OUTPUT_BYTES) child.kill("SIGKILL"); else chunks.push(chunk);
    });
    child.once("error", reject);
    child.once("exit", (code, signal) => code === 0 && signal === null && length <= MAX_OUTPUT_BYTES
      ? accept(Buffer.concat(chunks).toString("utf8"))
      : reject(new Error("Agent supply-chain Kubernetes runtime metadata probe failed")));
  });
}

function invalidInput() { throw new Error("Agent supply-chain runtime lock input is invalid"); }
function invalidLock() { throw new Error("Agent supply-chain runtime lock is invalid"); }

async function main() {
  if (process.env.NODE_ENV !== "production") invalidInput();
  const lock = await createAgentSupplyChainRuntimeLock(parseAgentSupplyChainRuntimeLockArguments(process.argv.slice(2)));
  process.stdout.write(`${JSON.stringify(lock)}\n`);
}

if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  main().catch(() => {
    process.stderr.write("[lock:agent-supply-chain-runtime] lock creation failed\n");
    process.exitCode = 1;
  });
}
