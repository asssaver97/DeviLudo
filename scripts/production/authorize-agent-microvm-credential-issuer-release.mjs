...
import {
  agentMicrovmCredentialIssuerReleaseSignerFromEnvironment,
...
} from "./agent-microvm-credential-issuer-release-authorization.mjs";
import { validateAgentMicrovmCredentialIssuerImageReceipt } from "./build-agent-microvm-credential-issuer-image.mjs";
import { renderAgentMicrovmCredentialIssuerRelease } from "./deploy-agent-microvm-credential-issuer.mjs";
...
export function parseAgentMicrovmCredentialIssuerReleaseAuthorizationArguments(argv) {
...
async function main() {
  if (process.env.NODE_ENV !== "production") {
    throw new Error("Agent microVM credential issuer release authorization requires production mode");
  }
  const root = resolve(dirname(fileURLToPath(import.meta.url)), "../..");
  const options = parseAgentMicrovmCredentialIssuerReleaseAuthorizationArguments(process.argv.slice(2));
  ...
  const policy = validateAgentMicrovmCredentialIssuerReleaseTrustPolicy(
    await readJson(options.trustPolicyPath), options.trustPolicyDigest);
  if (agentMicrovmCredentialIssuerReleaseTrustPolicyDigest(policy) !== options.trustPolicyDigest) invalid();
  ...
  const claims = createAgentMicrovmCredentialIssuerReleaseClaims(bundle, options.clusterContext, {
    ...(options.authorizationId === undefined ? {} : { authorizationId: options.authorizationId }),
    ttlSeconds: options.ttlSeconds,
  });
  const signer = await agentMicrovmCredentialIssuerReleaseSignerFromEnvironment();
...
function invalid() { throw new Error("Agent microVM credential issuer release authorization input is invalid"); }

if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  main().catch(() => { process.stderr.write("[authorize:agent-microvm-credential-issuer] authorization failed\n"); process.exitCode = 1; });
}
