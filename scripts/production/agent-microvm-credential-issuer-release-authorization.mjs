...
const CLAIM_KEYS = Object.freeze([
  ...
  "timeoutSeconds", "toolchainBaseImage",
]);
...
export function agentMicrovmCredentialIssuerReleaseTrustPolicyDigest(policy) {
  validateAgentMicrovmCredentialIssuerReleaseTrustPolicy(policy);
  return sha256Canonical(policy);
}

export function validateAgentMicrovmCredentialIssuerReleaseTrustPolicy(policy, expectedDigest) {
  if (!plainRecord(policy) || !exactKeys(policy, POLICY_KEYS)
    || policy.schemaVersion !== "deviludo.agent-microvm-credential-issuer-release-trust-policy.v1"
...
export function createAgentMicrovmCredentialIssuerReleaseClaims(bundle, clusterContext, {
  ...
  return Object.freeze({
    schemaVersion: "deviludo.agent-microvm-credential-issuer-release-claims.v1",
    authorizationId,
    ...
    runtimeLockDigest: agentMicrovmCredentialIssuerRuntimeLockDigest(bundle.runtimeLock),
    issuedAt: issuedAt.toISOString(),
    expiresAt: new Date(issuedAt.valueOf() + ttlSeconds * 1_000).toISOString(),
  });
}

export function agentMicrovmCredentialIssuerReleaseSigningRequest(claims) {
  validateClaimsShape(claims);
  return Object.freeze({
    schemaVersion: "deviludo.agent-microvm-credential-issuer-release-signing-request.v1",
...
export function agentMicrovmCredentialIssuerReleaseAuthorizationFromSigner(
  ...
  const trusted = validateAgentMicrovmCredentialIssuerReleaseTrustPolicy(policy, expectedPolicyDigest);
  ...
  if (!plainRecord(response) || !exactKeys(response, SIGNER_RESPONSE_KEYS)
    || response.schemaVersion !== "deviludo.agent-microvm-credential-issuer-release-signing-response.v1"
  ...
  verifyAgentMicrovmCredentialIssuerReleaseAuthorization(authorization, trusted, expectedPolicyDigest, {
    bundle, clusterContext: claims.clusterContext, now,
  });
  return authorization;
}

export function verifyAgentMicrovmCredentialIssuerReleaseAuthorization(authorization, policy, expectedPolicyDigest, {
  bundle, clusterContext, now = new Date(),
} = {}) {
  if (typeof expectedPolicyDigest !== "string" || !SHA256.test(expectedPolicyDigest)) invalidAuthorization();
  const trusted = validateAgentMicrovmCredentialIssuerReleaseTrustPolicy(policy, expectedPolicyDigest);
  if (!plainRecord(authorization) || !exactKeys(authorization, AUTHORIZATION_KEYS)
    || authorization.schemaVersion !== "deviludo.agent-microvm-credential-issuer-release-authorization.v1"
...
export class MtlsAgentMicrovmCredentialIssuerReleaseSigner {
  ...
  async sign(bundle, claims, policy, expectedPolicyDigest, now = new Date()) {
    ...
    const signingRequest = agentMicrovmCredentialIssuerReleaseSigningRequest(claims);
    const response = await this.request({
      url: new URL("/v1/agent-microvm-credential-issuer-releases/sign-ed25519", this.endpoint),
      tls: this.tls,
    ...
    if (response.statusCode !== 200 || response.body?.keyId !== this.keyId) {
      throw new Error("Agent microVM credential issuer release signing Broker rejected the request");
    }
    return agentMicrovmCredentialIssuerReleaseAuthorizationFromSigner(
      claims, response.body, bundle, trusted, expectedPolicyDigest, now);
  }
}

export async function agentMicrovmCredentialIssuerReleaseSignerFromEnvironment(env = process.env) {
  const [key, cert, ca] = await Promise.all([
    boundedFile(env.DEVILUDO_AGENT_CREDENTIAL_ISSUER_RELEASE_SIGNER_TLS_KEY_FILE),
    boundedFile(env.DEVILUDO_AGENT_CREDENTIAL_ISSUER_RELEASE_SIGNER_TLS_CERT_FILE),
    boundedFile(env.DEVILUDO_AGENT_CREDENTIAL_ISSUER_RELEASE_SIGNER_TLS_CA_FILE),
  ]);
  return new MtlsAgentMicrovmCredentialIssuerReleaseSigner({
    endpoint: env.DEVILUDO_AGENT_CREDENTIAL_ISSUER_RELEASE_SIGNER_ENDPOINT,
    keyId: env.DEVILUDO_AGENT_CREDENTIAL_ISSUER_RELEASE_SIGNING_KEY_ID,
    tls: { key, cert, ca },
  });
}
...
function validateClaimsShape(claims) {
  if (!plainRecord(claims) || !exactKeys(claims, CLAIM_KEYS)
    || claims.schemaVersion !== "deviludo.agent-microvm-credential-issuer-release-claims.v1"
...
function validateClaimsBinding(value, bundle, clusterContext, now) {
  ...
  if (!(now instanceof Date) || !Number.isFinite(now.valueOf()) || typeof clusterContext !== "string"
    ...
    || claims.runtimeLockDigest !== agentMicrovmCredentialIssuerRuntimeLockDigest(bundle.runtimeLock)) invalidAuthorization();
...
function validReleaseBundle(bundle) {
  try {
    validateAgentMicrovmCredentialIssuerImageReceipt(bundle?.receipt, {
      ...
      toolchainBaseImage: bundle?.receipt?.toolchainBaseImage, sourceRevision: bundle?.receipt?.sourceRevision,
      platform: bundle?.receipt?.platform,
    });
    validateAgentMicrovmCredentialIssuerRuntimeLock(bundle?.runtimeLock, {
...
async function requestSigner(input) { return new Promise((accept, reject) => {
  const request = httpsRequest(input.url, { method: "POST", key: input.tls.key, cert: input.tls.cert, ca: input.tls.ca,
    ...
    if (response.headers["content-type"] !== "application/json") { response.resume();
      reject(new Error("Agent microVM credential issuer release signing response content type is invalid")); return; }
    const chunks = []; let length = 0;
    response.on("data", (chunk) => { length += chunk.length;
      if (length > 16_384) request.destroy(new Error("Agent microVM credential issuer release signing response is too large"));
      else chunks.push(chunk); });
    response.on("end", () => { let body; try { body = JSON.parse(Buffer.concat(chunks).toString("utf8")); }
      catch { reject(new Error("Agent microVM credential issuer release signing response is invalid")); return; }
      accept({ statusCode: response.statusCode ?? 0, body }); });
  });
  request.once("timeout", () => request.destroy(new Error("Agent microVM credential issuer release signing request timed out")));
  request.once("error", reject); request.end(input.body);
}); }
function invalidPolicy() { throw new Error("Agent microVM credential issuer release trust policy is invalid"); }
function invalidAuthorization() { throw new Error("Agent microVM credential issuer release authorization is invalid"); }
function invalidSigner() { throw new Error("Agent microVM credential issuer release signing configuration is invalid"); }
