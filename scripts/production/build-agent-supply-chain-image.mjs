...
export function validateAgentSupplyChainImageSpec(input, platformVersion) {
  ...
  if (!node || !node.groups?.repository.endsWith("/node") || Number(node.groups.minor) < 13
    || !toolchain || !toolchain.groups?.repository.endsWith("/agent-supply-chain-toolchain")
    || !fixedVersion(toolchain.groups.version) || toolchain.groups.version !== platformVersion
    || !destination || !destination.groups?.repository.endsWith("/agent-supply-chain")
    || !SOURCE_REVISION.test(input.sourceRevision)
    || !PLATFORM.test(platform)) invalid();
...
export function agentSupplyChainImageBuildCommand(spec, metadataFile) {
  const value = validateAgentSupplyChainImageSpec(spec, spec?.platformVersion);
  if (typeof metadataFile !== "string" || !metadataFile.startsWith("/") || /[\0\r\n]/.test(metadataFile)) invalid();
  return Object.freeze({
    command: "docker",
    args: Object.freeze([
      "buildx", "build",
      "--file", "Dockerfile.agent-supply-chain",
...
export function parseAgentSupplyChainImageBuildMetadata(value) {
  const digest = plainRecord(value) ? value["containerimage.digest"] : undefined;
  if (typeof digest !== "string" || !IMAGE_DIGEST.test(digest)) {
    throw new Error("Agent supply-chain image digest is missing from BuildKit metadata");
  }
  return digest;
}

export function agentSupplyChainImageReceipt(spec, metadata, inputs, completedAt = new Date().toISOString()) {
  const value = validateAgentSupplyChainImageSpec(spec, spec?.platformVersion);
  const imageDigest = parseAgentSupplyChainImageBuildMetadata(metadata);
...
export function validateAgentSupplyChainImageReceipt(receipt, expected) {
  ...
  ]) || receipt.schemaVersion !== "deviludo.agent-supply-chain-image-receipt.v1"
  ...
  if (!new RegExp(`^${OCI_REPOSITORY}$`).test(reference) || !reference.endsWith("/agent-supply-chain")) invalidReceipt();
  return Object.freeze({ ...receipt, attestations: Object.freeze([...receipt.attestations]) });
}

export function parseAgentSupplyChainImageArguments(argv, platformVersion) {
  ...
  return validateAgentSupplyChainImageSpec({
...
async function main() {
  ...
  const spec = parseAgentSupplyChainImageArguments(process.argv.slice(2), packageJson.version);
  const temporary = await mkdtemp(join(tmpdir(), "deviludo-agent-supply-chain-image-"));
  const metadataFile = join(temporary, "metadata.json");
  try {
    const build = agentSupplyChainImageBuildCommand(spec, metadataFile);
    await run(build.command, build.args, root);
    const receipt = agentSupplyChainImageReceipt(spec, JSON.parse(await readFile(metadataFile, "utf8")), {
...
function run(command, args, cwd) {
  return new Promise((accept, reject) => {
    ...
    child.once("exit", (code, signal) => code === 0 && signal === null
      ? accept() : reject(new Error("Agent supply-chain image build failed")));
  });
}
...
function invalid() { throw new Error("Agent supply-chain image build input is invalid"); }
function invalidReceipt() { throw new Error("Agent supply-chain image receipt is invalid"); }

if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  main().catch(() => { process.stderr.write("[image:build-agent-supply-chain] build failed\n"); process.exitCode = 1; });
}
