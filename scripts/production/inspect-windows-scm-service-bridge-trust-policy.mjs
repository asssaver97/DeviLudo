...
export function parseWindowsScmServiceBridgeTrustInspectionArguments(argv) {
  if (!Array.isArray(argv) || argv.length !== 2 || argv[0] !== "--trust-policy") invalid();
  return Object.freeze({ trustPolicyPath: absolute(argv[1]) });
}

export function inspectWindowsScmServiceBridgeTrustPolicy(value) {
  const policy = validateWindowsScmServiceBridgeTrustPolicy(value);
  return Object.freeze({
    schemaVersion: "deviludo.windows-scm-service-bridge-trust-policy-inspection.v1",
    policyId: policy.policyId,
    policyRevision: policy.policyRevision,
    policyDigest: windowsScmServiceBridgeTrustPolicyDigest(policy),
...
async function main() {
  const options = parseWindowsScmServiceBridgeTrustInspectionArguments(process.argv.slice(2));
  ...
  process.stdout.write(`${JSON.stringify(inspectWindowsScmServiceBridgeTrustPolicy(value))}\n`);
}
...
function invalid() { throw new Error("Windows SCM service bridge trust inspection input is invalid"); }

if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  main().catch(() => {
    process.stderr.write("[inspect:windows-scm-service-bridge-trust] inspection failed\n");
    process.exitCode = 1;
  });
}
