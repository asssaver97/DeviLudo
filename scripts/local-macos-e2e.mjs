...
const stopPreparation = signal => preparationController.abort(new Error(`macOS E2E preparation received ${signal}`));
const preparationSignalHandlers = new Map();
for (const signal of ["SIGINT", "SIGTERM"]) {
  const handler = () => stopPreparation(signal);
  preparationSignalHandlers.set(signal, handler);
...
try {
  ...
  prepared = !preparationController.signal.aborted;
} catch (error) {
  if (preparationController.signal.aborted) {
    ...
    await reportPreparation({
      ...
      stage: "FAILED",
      progress: 100,
      message: `macOS E2E preparation failed: ${error instanceof Error ? error.message : String(error)}`.slice(0, 240),
    });
    throw error;
  }
} finally {
  for (const [signal, handler] of preparationSignalHandlers) process.off(signal, handler);
}
if (prepared && !preparationController.signal.aborted) {
  ...
  return payload => {
    ...
    queue = queue.then(async () => {
      try {
        ...
        console.warn(`[DeviLudo:E2E] Failed to report preparation progress: ${error instanceof Error ? error.message : String(error)}`);
      }
    });
    return queue;
  };
