...
- `services/agent-supply-chain`：独立 mTLS Agent 供应链 Broker；固定原生执行接口只接受官方精确版本，持久化成功或终态失败回执。签名/扫描失败自动拒绝版本，镜像/canary 失败自动隔离安装、停止新任务并恢复上一健康 Profile；普通网络或扫描器故障不会被误判为安全终态。
