...
- `/settings/agents`：独立部署时，TenantAdmin 可写入租户 BYOK、创建第三方 Provider/Profile 草稿并选择租户默认；API Key 明文仅进入 Vault。作为 DeviLudo-Platform submodule 启动时该页面和 API 均关闭，Workspace 不会看到 API Key、Base URL 或执行服务器配置。
- `/admin/agents`：共享实现的全局运维台，集中配置 Claude Code/Codex Provider、API Key、Agent 开发服务器与 Linux/Windows/macOS E2E 服务器；Platform 托管模式只接受账号服务签发的 `PlatformAgentAdmin`、`SecurityAdmin` 或 `Auditor` 权限。
