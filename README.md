...
- `services/user-acceptance`：生产候选反馈/接受入口只接受平台签名会话经 mTLS Web workload 转发的用户决定；服务端解析唯一等待中的验收 action、带第 3 次失败修复上下文的规格批准 action，或已撤销发布权限的 main/Steam 失败 action。反馈持久化 `GENERATING → DRAFT_READY → COMPLETED` 并创建沿用同一 aggregate 的下一对 DRAFT 规格/测试计划及新对话；候选反馈会原子失效旧证据，人工修复接管则验证失败 action 与草稿祖先而不伪造候选证据。接受操作仍须先记录不可变 actor、候选回执、commit、PR 与有效证据绑定，再发出 `USER_ACCEPTED` 供 SCM 合并。模型失败不失效证据，投递失败不重复生成草案或用户决定。
