...
- `services/user-acceptance`：生产候选反馈入口只接受平台签名会话经 mTLS Web workload 转发的用户文本；服务端解析唯一等待中的验收 action，持久化 `GENERATING → DRAFT_READY → COMPLETED`，创建沿用同一 aggregate 的下一对 DRAFT 规格/测试计划及新对话后，才由控制面原子失效旧候选证据并续跑 Temporal。模型失败不失效证据，投递失败不重复生成草案。
