...
- `services/local-agent-runtime`：仅 loopback 的 Agent 就绪与执行边界；读取本机 Claude Code/Codex CLI 的精确版本，并把版本、WorkerImage、Gateway、锁定 Provider 绑定探针和显式启用状态作为联合门禁。本地启动器还会在该 sidecar 内启用进程级 Provider 密钥保管区：后台通过带正文摘要和单次 nonce 的 HMAC 请求写入 Key，D1 只得到 `SecretRef`/指纹；严格探针直接复用生产 Gateway 的 HTTPS、DNS/CNAME 固定、重定向复检、模型/流式/工具/取消/usage/超时验证。Key 不进入 CLI、工作区或日志，sidecar 重启即销毁并要求重新登记。`/v1/runs` 必须复用预检，拒绝未知字段和调用方工作区路径；Web 只能从已批准规格侧车读取并复核 GameSpec/TestPlan 双摘要，再生成带 SHA-256 的完整提示词。请求、执行器和完成回执共同绑定该提示词摘要。相同 Run/Attempt 的并发请求只执行一次；用户取消、Web 断连和服务退出会沿 AbortSignal 传播到 CLI 进程监督器，独立签名取消端点以精确租户/项目/运行绑定和有界 tombstone 阻止迟到预检在交付取消后启动。只有绑定回执通过后，本地 D1 才把必填取消原因、请求时间和 Agent 撤销状态写入不可变快照与事件。默认未注入隔离执行器时返回 503，绝不回退为直接启动 CLI；生产仍使用独立 mTLS Secret Broker 与 Vault/KMS，不把本地进程级保管区当作生产密钥库。
...
打开 `http://127.0.0.1:3000`。该命令同时启动 Web 控制面、`127.0.0.1:4311` Godot 验证侧车、`127.0.0.1:4312` Agent 就绪探针和 `127.0.0.1:4313` 确定性规格对话侧车，四个进程都只绑定 loopback。启动器为三个 sidecar 分别生成临时 HMAC Key；每个 Key 只进入 Web 与对应 sidecar，并以 `0600` 权限写入被忽略的 `.deviludo/` 供本地冒烟验证，退出时全部删除。规格对话、批准、反馈分支和已完成幂等回执另存为不含 Key 的 `0600` 原子状态文件，服务重启后恢复；文件损坏、权限放宽或符号链接替换会令规格侧车拒绝启动。启动器还持有不含密钥的原子部署租约；四个受监督子进程持续核对 launcher PID 和随机 deployment ID。即使 launcher 被强制终止，子进程也会自行退出释放端口，下一次启动在安全窗口后自动废弃旧会话文件并生成全新密钥，不需要人工清理。规格/项目产品页面和 D1 持久状态不会调用真实模型、GitHub 或 Steam；只有 SecurityAdmin 在 Provider 表单明确提交“测试并激活”时，Agent sidecar 才使用进程内 Key 运行严格上游探针。项目页的“真实本机验证”会运行已安装的 Godot，并把证据写入被忽略的 `.deviludo/`。Agent 探针只读取 `claude --version` / `codex --version`；若版本不等于任务锁定值，管理员页会如实显示 `VERSION_MISMATCH` 并阻止执行。
