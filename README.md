...
- `services/local-agent-runtime`：仅 loopback 的 Agent 就绪与执行边界；本地启动器组合平台 Godot scaffold、真实 CLI 安全监督器、工作区外 SCM 代理和 `127.0.0.1:4314` 内部 Inference Gateway。Provider Key 通过逐请求 HMAC 写入进程级保管区，D1 只得到 `SecretRef`/指纹；严格探针与独立激活分离，激活后才可签发最长 15 分钟、绑定 tenant/project/run/profile/provider/credential/models/budget 的 DLRT。CLI 只拿 DLRT 并访问 loopback Gateway，上游 Key 不进入 CLI、工作区、命令行或日志；Gateway 复用生产 DNS/CNAME 固定、HTTPS/重定向复检、模型白名单、usage claim 和计价账本。Attempt 终止即撤销令牌，sidecar 重启清空全部明文与激活绑定。`/v1/runs` 复用版本、WorkerImage、Provider、模型和执行授权门禁，拒绝未知字段及调用方工作区；完成候选还必须通过保留文件代码评审和 SCM 摘要绑定。该本地进程隔离不冒充生产 microVM；生产仍使用 mTLS Secret Broker、Vault/KMS 和独立 Gateway。
