const PLATFORM_ROLES = new Set(["PlatformAgentAdmin", "SecurityAdmin"]);
const SAFE_SUBJECT = /^[A-Za-z0-9][A-Za-z0-9._:@/-]{0,159}$/;
const UUID = /^[a-f0-9]{8}-[a-f0-9]{4}-[1-5][a-f0-9]{3}-[89ab][a-f0-9]{3}-[a-f0-9]{12}$/;
const SIGNATURE = /^[A-Za-z0-9_-]{43}$/;

export interface TrustedPlatformAdmin {
  readonly role: "PlatformAgentAdmin" | "SecurityAdmin";
...
export async function verifyTrustedPlatformAdmin(
  ...
  now = new Date(),
): Promise<TrustedPlatformAdmin> {
  const url = new URL(request.url);
  if (url.search || url.pathname !== "/api/admin/invitations") invalid();
  ...
  return Object.freeze({ role: role as TrustedPlatformAdmin["role"], actorId, sessionId });
}
