...
export type DeliverySignal =
  ...
  | { type: "EXTERNAL_APPROVAL_NEEDED"; gate: "VALVE_REVIEW" | "FIRST_RELEASE" | "DEFAULT_BRANCH_CONFIRMATION" }
...
export interface DeliverySnapshot {
  ...
  readonly externalGate: string | null;
...
export class GameDeliveryWorkflow {
  ...
  nextCommand(): DeliveryCommand {
    const commands: Record<DeliveryState, DeliveryCommand> = {
      ...
      RELEASED: "PUBLISH_STEAM_DEFAULT_BRANCH",
      CANCELLED: "NONE",
    };
  ...
  signal(signal: DeliverySignal): DeepReadonly<DeliverySnapshot> {
    ...
    switch (this.snapshot.state) {
      ...
      case "DEVELOPING":
        ...
        if (signal.type === "AGENT_COMPLETED") return this.commit(signal, { state: "CROSS_PLATFORM_E2E", candidateCommitSha: signal.candidateCommitSha });
      ...
      case "CROSS_PLATFORM_E2E":
        if (signal.type === "E2E_PASSED") return this.commit(signal, { state: "WAITING_USER_ACCEPTANCE", evidenceBundleId: signal.evidenceBundleId });
      ...
      case "MAIN_SHA_E2E":
        return signal.type === "E2E_PASSED" ? this.commit(signal, { state: "WAITING_MFA", evidenceBundleId: signal.evidenceBundleId }) : this.invalid(signal);
      case "WAITING_MFA":
        return signal.type === "MFA_APPROVED" ? this.commit(signal, { state: "STEAM_PRIVATE_BETA" }) : this.invalid(signal);
      case "STEAM_PRIVATE_BETA":
        return signal.type === "BETA_ACTIVATED" ? this.commit(signal, { state: "STEAM_INSTALL_E2E" }) : this.invalid(signal);
      case "STEAM_INSTALL_E2E":
        if (signal.type === "STEAM_INSTALL_PASSED") return this.commit(signal, { state: "EXTERNAL_APPROVAL_REQUIRED", evidenceBundleId: signal.evidenceBundleId, externalGate: "FIRST_RELEASE" });
      ...
      case "EXTERNAL_APPROVAL_REQUIRED":
        return signal.type === "EXTERNAL_APPROVED" ? this.commit(signal, { state: "RELEASED", externalGate: null }) : this.invalid(signal);
      case "RELEASED":
        return this.invalid(signal);
    }
  }
