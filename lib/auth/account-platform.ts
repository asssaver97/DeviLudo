...
const ADMIN_ROLES=new Set(["PlatformAgentAdmin","SecurityAdmin","Auditor"]);
...
export interface AccountPlatformSession {
  ...
  readonly platformAdminRoles:readonly ("PlatformAgentAdmin"|"SecurityAdmin"|"Auditor")[];
}
...
function adminRoles(value:unknown):AccountPlatformSession["platformAdminRoles"]{if(value===undefined)return Object.freeze([]);if(!Array.isArray(value)||value.length>ADMIN_ROLES.size||value.some(role=>typeof role!=="string"||!ADMIN_ROLES.has(role))||new Set(value).size!==value.length)throw new Error("Account Platform response is invalid");return Object.freeze([...value] as AccountPlatformSession["platformAdminRoles"]);}
