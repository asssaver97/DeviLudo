...
import { parseEvidenceCatalogProjection, type EvidenceCatalogProjection } from "@/lib/evidence/catalog-projection";
...
export class DeliveryProjectionBrokerClient {
  ...
  async readEvidenceCatalog(input: { readonly tenantId: string; readonly projectId: string }): Promise<EvidenceCatalogProjection> {
    if (!UUID.test(input.tenantId) || !UUID.test(input.projectId)) throw new Error("Evidence catalog read binding is invalid");
    const response = await this.fetcher(new URL(`/v1/evidence-catalog/${encodeURIComponent(input.projectId)}`, this.#origin), {
      method: "GET",
      redirect: "manual",
      headers: { accept: "application/json", "x-deviludo-tenant-id": input.tenantId },
      cache: "no-store",
    ...
    const envelope = object(value);
    if (Object.keys(envelope).length !== 1) invalid();
    try { return await parseEvidenceCatalogProjection(envelope.data, input); }
    catch { invalid(); }
  }
}
