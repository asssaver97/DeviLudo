...
3. Ask one compact batch of at most three high-leverage questions per turn. Offer concrete, contrasting choices and a recommended default when useful; use `options` for replies the player can select directly. Do not conduct a long questionnaire or ask again for a decision already present in context.
...
Use `e2eGoalDelta.add`, `replace`, and `retire` deliberately. Explicitly name goals replaced or retired by the approved proposal; never silently weaken coverage. The `implementationBrief` must preserve resolved rules, system interactions, invariants, tunables, risks, exclusions, and the smallest playable slice that do not fit naturally in the player-facing summary.
