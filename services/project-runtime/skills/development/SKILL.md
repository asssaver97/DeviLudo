...
For a read-only conversation branch, end with one JSON object containing `content`, `readyForDevelopment`, `options`, `implementationBrief`, `projectDocumentPatch`, and `e2eGoalDelta`; the document patch and goal delta are empty. For a primary workflow turn, end with `{"content":"...","sourceRevision":N,"handoff":{"toRole":"TEST","sourceRevision":N,"summary":"..."}}` only after the checkpoint, build request, and handoff tool calls succeed. `content` is a concise player-facing completion message explaining what was generated and that controlled build and testing follow; do not expose internal paths, prompts, or raw logs.
