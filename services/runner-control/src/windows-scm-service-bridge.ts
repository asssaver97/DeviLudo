...
export interface WindowsScmServiceBridgeTrustPolicy {
  readonly schemaVersion: "deviludo.windows-scm-service-bridge-trust-policy.v1";
...
export interface WindowsScmServiceBridgeClaims {
  readonly kind: "deviludo-windows-scm-service-bridge";
  readonly version: 1;
  readonly revision: number;
  readonly platform: "windows";
  readonly architecture: "x86_64" | "arm64";
  readonly bridgeVersion: string;
  readonly serviceContractVersion: 1;
...
export interface SignedWindowsScmServiceBridgeManifest {
  readonly keyId: string;
  readonly claims: WindowsScmServiceBridgeClaims;
  readonly signature: string;
}

export function windowsScmServiceBridgeTrustPolicyDigest(value: unknown): string {
  return sha256Canonical(validateWindowsScmServiceBridgeTrustPolicy(value));
}

export function validateWindowsScmServiceBridgeTrustPolicy(
  value: unknown,
  expectedDigest?: string,
): WindowsScmServiceBridgeTrustPolicy {
  const policy = record(value) as unknown as WindowsScmServiceBridgeTrustPolicy;
  exactKeys(record(policy), ["schemaVersion", "policyId", "policyRevision", "keys"]);
  if (policy.schemaVersion !== "deviludo.windows-scm-service-bridge-trust-policy.v1"
  ...
  const keys = policy.keys.map((candidate) => {
    const key = record(candidate) as unknown as WindowsScmServiceBridgeTrustPolicy["keys"][number];
  ...
  const trusted = deepFreeze({ ...policy, keys }) as WindowsScmServiceBridgeTrustPolicy;
  if (expectedDigest !== undefined && (!SHA256.test(expectedDigest) || sha256Canonical(trusted) !== expectedDigest)) invalidPolicy();
  return trusted;
}

export function verifySignedWindowsScmServiceBridgeManifest(
  value: unknown,
  options: Readonly<{
    trustPolicy: unknown;
...
): WindowsScmServiceBridgeClaims {
  const now = options.now ?? new Date();
  if (!new Set(["x86_64", "arm64"]).has(options.architecture) || !Number.isFinite(now.valueOf())) invalid();
  const trustPolicy = validateWindowsScmServiceBridgeTrustPolicy(options.trustPolicy, options.trustPolicyDigest);
  const envelope = record(value);
  exactKeys(envelope, ["keyId", "claims", "signature"]);
  const claims = record(envelope.claims) as unknown as WindowsScmServiceBridgeClaims;
  ...
  exactKeys(record(claims), [
    "architecture", "binaryDigest", "bridgeVersion", "builtAt", "kind", "platform", "revision",
    "serviceContractVersion", "sourceDigest", "supplyChainEvidenceDigest", "version",
  ]);
  const builtAt = Date.parse(claims.builtAt);
  if (claims.kind !== "deviludo-windows-scm-service-bridge" || claims.version !== 1
    || !Number.isSafeInteger(claims.revision) || claims.revision < 1 || claims.platform !== "windows"
    || claims.architecture !== options.architecture || !fixedVersion(claims.bridgeVersion)
    || claims.serviceContractVersion !== 1 || !SHA256.test(claims.binaryDigest)
...
function invalid(): never { throw new Error("Windows SCM service bridge manifest is invalid"); }
function invalidPolicy(): never { throw new Error("Windows SCM service bridge trust policy is invalid"); }
