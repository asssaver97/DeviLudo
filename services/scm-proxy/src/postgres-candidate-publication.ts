  ...
  async resolve(request: CandidatePublicationRequest) {
    return this.#transaction(request.tenantId, async (client) => {
      ...
      if (row.tenant_id !== request.tenantId || row.project_id !== request.projectId || row.run_id !== request.runId
        ...
        || lock.sourceBaselineReceiptId !== row.source_baseline_receipt_id || lock.commitSha !== row.baseline_commit_sha
        || artifact.specRevisionId !== row.spec_revision_id || artifact.expectedBaseCommitSha !== row.baseline_commit_sha) invalid();
