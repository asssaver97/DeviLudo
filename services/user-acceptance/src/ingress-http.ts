...
export interface UserAcceptanceHttpRequest {
...
export interface UserAcceptanceHttpResponse {
...
export function createUserAcceptanceHandler(options: {
  ...
  if (!options.allowedSpiffeIds.size) throw new Error("User acceptance workload allow-list is empty");
  const extractIdentity = options.extractIdentity ?? evidenceArchiveIdentityFromTlsSocket;
  return async (request: UserAcceptanceHttpRequest): Promise<UserAcceptanceHttpResponse> => {
    ...
    if (contentType(request.headers["content-type"]) !== "application/json") {
      return failure(415, "USER_ACCEPTANCE_JSON_REQUIRED");
    }
    let body: unknown;
...
export function createUserAcceptanceHttpsServer(options: {
  readonly tls: Pick<ServerOptions, "key" | "cert" | "ca">;
  readonly handler: (request: UserAcceptanceHttpRequest) => Promise<UserAcceptanceHttpResponse>;
}): HttpsServer {
  if (!options.tls.key || !options.tls.cert || !options.tls.ca) throw new Error("User acceptance TLS material is incomplete");
...
async function dispatch(
  ...
  handler: (request: UserAcceptanceHttpRequest) => Promise<UserAcceptanceHttpResponse>,
) {
  try {
    ...
    send(response, failure(
      error instanceof BodyLimitError ? 413 : 500,
      error instanceof BodyLimitError ? "USER_ACCEPTANCE_REQUEST_TOO_LARGE" : "USER_ACCEPTANCE_UNAVAILABLE",
    ));
  }
}
...
function send(response: ServerResponse, result: UserAcceptanceHttpResponse): void {
...
function failure(status: number, code: string): UserAcceptanceHttpResponse {
