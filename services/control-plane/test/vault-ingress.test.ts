...
test("control-plane Vault ingress uses fixed mTLS routes and wipes copied plaintext", async () => {
  ...
  const http: VaultIngressHttp = async (url, input) => {
    bodies.push(input.body); calls.push({ path: url.pathname, headers: input.headers });
  ...
  assert.equal(calls[0]?.path, "/secrets:write");
  assert.match(calls[0]?.headers["idempotency-key"] ?? "", /^[a-f0-9]{64}$/);
  await vault.revoke(secretRef);
  assert.ok(bodies[1]?.every((value) => value === 0));
  assert.equal(calls[1]?.path, "/secrets:revoke");
});
