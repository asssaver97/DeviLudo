...
function workflowTestConversationContent(
  ...
  const conclusion = responseLanguage === "zh"
    ? verdict === "PASS"
      ...
      : verdict === "REPLAN"
        ? "E2E 发现冻结计划引用了项目未发布的 Probe 字段，现已返回测试规划阶段重新生成计划，不会触发开发 Agent。"
      : verdict === "BLOCKED"
    ...
    : verdict === "PASS"
      ...
      : verdict === "REPLAN"
        ? "E2E found that the frozen plan references a Probe field not published by the project. Test planning is regenerating the plan without involving Development."
      : verdict === "BLOCKED"
