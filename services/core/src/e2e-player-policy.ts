...
function policyPrompt(request: PlayerPolicyRequest): string {
  return [
    ...
    "Treat visibly incomplete production presentation as PRODUCT_DEFECT: blank or dash-only asset slots, missing-image placeholders, atlas sheets shown without correct cropping, tiny unreadable textures inside oversized boxes, clipped/overlapping labels, or a raw debug/state dump replacing a composed player-facing HUD. A dense interface is valid only when the current objective and next enabled gameplay action remain visually clear and the artwork is correctly placed in its intended context.",
    ...
    "Take one state-changing gameplay action per decision so the runner can independently verify each player operation. Entering a session is setup, not completion: after the game becomes playable, exercise the visible primary action, its resulting choice or movement when applicable, and the loop-completion action before reporting GOAL_REACHED.",
