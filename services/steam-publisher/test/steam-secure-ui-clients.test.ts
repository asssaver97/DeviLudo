...
test("Secure UI access client forwards binary secrets and a resource capability to fixed Broker routes", async () => {
  const calls: Array<{ path: string; contentType?: string; body: Uint8Array | undefined; token?: string }> = [];
  ...
  const http: SteamAccessBinaryHttp = async (input) => {
    calls.push({ path: input.url.pathname, contentType: input.headers?.["content-type"], body: input.body,
      token: input.headers?.["x-deviludo-steam-ui-session"] });
  ...
  assert.deepEqual(calls.map((entry) => entry.path), [
    ...
    "/v1/mfa/approvals/approval-001/complete",
  ]);
  ...
  assert.doesNotMatch(JSON.stringify(calls.map(({ path, contentType, token }) => ({ path, contentType, tokenLength: token?.length }))), /not-a-real-password|ABC123|opaque/);
});
